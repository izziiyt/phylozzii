...
object EM{
  ...
  private def mStep(pt:PhylogencyTree,counts:List[Count]):PhylogencyTree = {
  ...
  private def newB(Ns:DenseMatrix[Double],Td:DenseVector[Double],m:EvolutionModel) = {
    val u = for(i <- 0 to 2;j <- i+1 to 3) yield Ns(i,j) + Ns(j,i)
    val v = for(i <- 0 to 2;j <- i+1 to 3) yield m.pi(j) * Td(i) + m.pi(i) * Td(j)
    calcNewParameter(u.toList,v.toList)
  }

  private def newT(Ns:List[DenseMatrix[Double]],Fd:List[DenseVector[Double]],m:EvolutionModel):List[Double] = {
    val Ns0:List[Double] = Ns.map(x => sum(x) - trace(x))
    (Ns0,Fd).zipped.map((ns0,fd) => - ns0 / (0 to 3).foldLeft(0.0)((x,i) => x + m.R(i,i) * fd(i)))
  }

  private def calcNewParameter(u:List[Double],v:List[Double]):DenseVector[Double] = {
    @tailrec

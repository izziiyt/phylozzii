...
class FastaTest extends FunSuite {
  ...
  test("sample.aa and sample.nuc") {
    val aa = "src/test/resources/phylozzii.fdur.util/sample.aa"
    val nuc = "src/test/resources/phylozzii.fdur.util/sample.nuc"
    val species = "src/test/resources/phylozzii.fdur.util/sample.species.txt"
    val nh = "src/test/resources/hg19.100way.nh"
    val cdn = "src/test/resources/codon.table.txt"
    val target = "target/result.txt"
    FdFilter.fdfilter(nuc,aa,target,nh,species,cdn)
    //23 32 35
